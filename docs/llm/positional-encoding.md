...
2. **相对位置可表示**：`PE(pos+k)` 是 `PE(pos)` 的线性函数（三角恒等式），理论上模型能学到相对距离；
3. 不同频率组合保证每个位置向量唯一；
...
- BERT：把位置编码当**可训练参数**（`n_positions × d_model` 的嵌入表），随模型一起学习；
...
```
q_i^T k_j = (R_i q)^T (R_j k) = q^T R_{j-i} k
```

- `R_i` 是分块旋转矩阵（对向量按 2 维一组做旋转，旋转角 = i × θ，θ 按频率递减）；
- 旋转角度随位置线性增长，频率类似正余弦的多频设计；
- 关键性质：**旋转后点积 = 相对距离的函数**，天然具备相对位置语义；
- 实现时用复数乘法或 `[x, -y; y, x]` 分块矩阵，无需额外参数。
...
| 可学习 PE | 绝对 | 有（n×d 表） | 差（查表止于训练长度） | BERT、GPT-2 |
