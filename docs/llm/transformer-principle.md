...
| Transformer | 注意力机制直接建模**任意两个位置**的关系 | 计算量大（O(n²)）；无位置信息需额外编码 |
...
把离散 token（词/子词）映射为连续向量。论文里 embedding 维度 `d_model = 512`，embedding 权重与最后的 softmax 前线性层（输出投影）**共享权重**，且在 embedding 时乘以 `√d_model`（防止点积值过大）。
...
- 周期递推：`PE(pos+k)` 可由 `PE(pos)` 线性表示，便于模型学习相对位置；
...
输入 `X ∈ R^(n×d_model)`，通过三个可学习矩阵 `W_Q, W_K, W_V` 得到：

```
Q = X W_Q    # Query 查询：我要找什么
K = X W_K    # Key   键：我是什么
V = X W_V    # Value 值：我提供什么内容
```
...
1. `Q K^T`：每个 query 与所有 key 点积，得到"相似度"分数矩阵 `n×n`；
2. `/ √d_k`：**缩放**。当 `d_k` 较大时点积值会过大，softmax 梯度趋于 0（梯度消失），除以 `√d_k` 把方差拉回 1 附近；
3. `softmax`：按行归一化成概率分布（每行和为 1），即注意力权重；
4. `× V`：加权求和，得到每个位置的输出。
...
点积 `Q K^T` 中，若 `q, k` 各分量独立且方差为 1，则点积的方差为 `d_k`，即数值随维度增长而变大。softmax 对过大输入会输出极端分布（接近 one-hot），梯度极小。除以 `√d_k` 保证方差为 1，softmax 梯度稳定——**这是论文里最容易被问到的细节之一**。
...
- 序列短（n < d）：自注意力占主导；
- 序列长（n ≫ d）：n² 项爆炸 → 催生了 FlashAttention、稀疏注意力、线性注意力（见第 9 节）。
...
单头注意力的"注意力模式"单一，且 `d_model=512` 的点积空间里，不同语义子空间（语法、指代、位置……）互相干扰。多头把 Q/K/V 拆成 `h=8` 个头，每头维度 `d_k = d_v = d_model/h = 64`，**并行**做注意力，再拼接回 `d_model`：
...
- 中间维度 `d_ff = 2048`，约为 `d_model` 的 4 倍；
...
Decoder 生成时**不能看到未来 token**。自注意力计算前，把 `Q K^T` 矩阵**右上三角（未来位置）设为 -∞**，softmax 后权重为 0，实现因果性（causal）：
...
- **Adam 优化器**，`β1=0.9, β2=0.98, ε=1e-9`；
- **学习率 warmup**：先线性增长到 `d_model^-0.5`（4000 步），再按步数平方根倒数衰减——防止早期训练不稳定；
- **Dropout**：子层输出、注意力权重、embedding 后均用 `p=0.1`；
- **Label Smoothing**：`ε=0.1`，缓解过拟合、提升 BLEU（但会降低困惑度）；
...
| 稀疏/线性注意力 | Longformer、Linformer、Sparse Transformer | 缓解 O(n²) 的序列长度瓶颈 |
...
2. **为什么除以 √d_k？** → 防点积过大导致 softmax 梯度消失（详见 4.2）。
...
7. **Transformer 复杂度？** → 注意力 O(n²d)、FFN O(nd²)、路径 O(1)（详见 4.3）。
